...
describe('CarbonFootPrint.service', () => {
  ...
  it('should compute the carbon foot print by id', async () => {
    ...
    if (productHamCheesePizza_kg) {
      const cfpHamCheesePizza_kg = await service.computationFromProductId(productHamCheesePizza_kg.id)

      expect(cfpHamCheesePizza_kg?.emissionCO2).toBe(0.1 * 0.11 + 0.15 * 0.12 + 0.4 * 0.13 + 0.7 * 0.14 + 0.3 * 0.15)
    }
    ;
  ...
  it('Should compute  the carbon food print from a new product and insert the product in the database', async () => {
    ...
    const cfpHamCheesePizza_gr = await service.computationFromProduct(hamCheesePizza_gr)
    const product = await dataSource.getRepository(Product).findOne({ where: { name: "hamCheesePizza_gr" } })

    expect(cfpHamCheesePizza_gr?.emissionCO2).toBe(((0.1 / 1000) * 0.11 + (0.15 / 1000) * 0.12 + (0.4 / 1000) * 0.13 + (0.7 / 1000) * 0.14 + (0.3 / 1000) * 0.15))
  ...
  it('Should throw an error if the unit is not correct', async () => {
    await expect(service.Compute_carbon_foot_print(hamCheesePizza_l.ingredients)).rejects.toThrow(Error)
      ;
  })

  it('Should set the result to null if one carbon factor is missing', async () => {
    expect(await service.Compute_carbon_foot_print(burger.ingredients)).toBeNull()
      ;
  })
